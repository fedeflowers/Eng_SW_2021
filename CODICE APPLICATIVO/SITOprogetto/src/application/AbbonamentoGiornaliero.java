...
public class AbbonamentoGiornaliero extends Abbonamento {
	...
	//per abbonamento giornaliero e settimanale siccome non so quando andr‡ ad attivarli dico che la carta deve avere validit‡ almeno un mese di scarto da quando viene creato l'abbonamento
	//successivamente se la cart‡ sar‡ scaduta durante l'attivazione dell'abbonamento non sar‡ possibile noleggiare
	...
	public boolean rinnovaAbb(String nomeUtente) {
		...
		c.add(Calendar.DATE, 30);
		dataValidit‡MassimaCarta = convertUtilToSql(c.getTime()); // data odierna + 30
		if (abbonamentoDao.getValidit‡Carta(nomeUtente).after(dataValidit‡MassimaCarta)) { // allora scade dopo quindi ok
